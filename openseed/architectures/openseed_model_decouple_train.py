...
class OpenSeeD(nn.Module):
    ...
    def forward_seg(self, batched_inputs, task='seg'):
        ...
        if self.training:
            ...
            mask_feature_results = outputs["decoder_output"]
            if 'seg' in task:
                if task == 'seg':
            ...
            for mask_cls_result, mask_pred_result, mask_feature_result,mask_box_result, input_per_image, image_size in zip(
                mask_cls_results, mask_pred_results, mask_feature_results,mask_box_results, batched_inputs, images.image_sizes
            ):
                ...
                if self.sem_seg_postprocess_before_inference:
                    ...
                    mask_feature_result = mask_feature_result.to(mask_pred_result)
                ...
                if self.panoptic_on:
                    ...
                    panoptic_r = retry_if_cuda_oom(self.panoptic_inference)(mask_cls_result, mask_pred_result,mask_feature_result,mask_box_result)
    ...
    def panoptic_inference(self, mask_cls, mask_pred,mask_feature,mask_box):
        ...
        cur_features = mask_feature[keep]
        cur_box = mask_box[keep]
                    ...
                    segments_info.append(
                        {
                            ...
                            "semantic_feature":cur_features[k],
                            "bbox":cur_box[k]
                        }
                    )
